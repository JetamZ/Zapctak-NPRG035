...
namespace AsciiArtGeneration
{
    public class InteractiveConfigBuilder {
        ...
        private static void askStore(GeneratorConfig config) {
            Console.WriteLine("Would you like to save your configuration ? [Y/n]");
            ...
            if (storing) {
                ...
                if (configStorePath != "") {
                    ...
                    File.WriteAllText(Path.Combine("config", "CustomConfig.txt"), config.ToString());
                }
            }
        }
        ...
        private static string[] askSymbols() { 
            ...
            if (response.ToLower().Trim() == "auto") {
                ...
                return BestSymbolPatternFinder.findBestPattern(4, num, chars).toStringArray();
            } else {
